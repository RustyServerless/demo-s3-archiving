...
pub fn local_header(e: &EntryMeta) -> Vec<u8> {
    debug_assert!(e.size <= U32_MAX as u64, "entry size must fit 32 bits in STORE local header");
...
pub fn central_dir_entry(e: &EntryMeta) -> Vec<u8> {
    debug_assert!(e.size <= U32_MAX as u64, "entry size must fit 32 bits in STORE central record");
    let name = e.name.as_bytes();
...
pub fn central_dir_entry_len(name: &str, local_header_offset: u64) -> u64 {
    let extra = if local_header_offset > U32_MAX as u64 { 12 } else { 0 };
...
pub fn end_records(entry_count: u64, cd_offset: u64, cd_size: u64) -> Vec<u8> {
    ...
    p16(&mut b, 0);
    p16(&mut b, 0);
    p16(&mut b, if entry_count >= U16_MAX as u64 { U16_MAX } else { entry_count as u16 });
    p16(&mut b, if entry_count >= U16_MAX as u64 { U16_MAX } else { entry_count as u16 });
    p32(&mut b, if cd_size >= U32_MAX as u64 { U32_MAX } else { cd_size as u32 });
    p32(&mut b, if cd_offset >= U32_MAX as u64 { U32_MAX } else { cd_offset as u32 });
    p16(&mut b, 0); // comment len
...
mod tests {
    ...
    fn meta(name: &str, size: u64, crc: u32, off: u64) -> EntryMeta {
        EntryMeta { name: name.to_string(), size, crc, local_header_offset: off }
    }
    ...
    fn local_header_fixed_fields_and_len() {
        ...
        assert_eq!(u32::from_le_bytes([h[14], h[15], h[16], h[17]]), 0xDEAD_BEEF);
    ...
    fn central_no_zip64_when_offset_small() {
        ...
        assert_eq!(c.len() as u64, central_dir_entry_len(&e.name, e.local_header_offset));
    ...
    fn end_records_sizes_and_sentinels() {
        ...
        assert_eq!(u64::from_le_bytes(r[48..56].try_into().unwrap()), 14_000_000_000); // cd offset
        // locator
        let loc = &r[56..76];
        assert_eq!(&loc[0..4], &SIG_ZIP64_LOCATOR.to_le_bytes());
        assert_eq!(u64::from_le_bytes(loc[8..16].try_into().unwrap()), 14_000_000_000 + 200_000);
